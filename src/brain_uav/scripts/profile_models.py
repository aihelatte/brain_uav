...
def describe_snn(checkpoint: Path, sample_count: int = 32) -> tuple[float, float, float, float, float]:
    cfg = ExperimentConfig()
    env = make_env(cfg, seed=cfg.training.seed)
    obs, _ = env.reset(seed=cfg.training.seed)
    ...
    return float(dense_macs), float(params), float(sum(effective_macs) / len(effective_macs)), float(sum(spike_rate_l1) / len(spike_rate_l1)), float(sum(spike_rate_l2) / len(spike_rate_l2))
...
def main() -> None:
    ...
    snn_dense_macs, snn_params, snn_effective_macs, spike_rate_l1, spike_rate_l2 = describe_snn(args.snn)
